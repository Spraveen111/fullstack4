...
app.post("/InsertVegitableData", async (req, res) => {
  ...
  try {
    ...
    const insertResults = await Promise.all(dataArray.map(async (data) => {
      return await vegitable_data.insertOne(data);
    }));

    console.log("Successfully inserted into vegitable_data", insertResults);
    res.status(200).json({ message: "Data inserted successfully into vegitable_data", data: insertResults });
  } catch (error) {
    console.error("Error inserting data into vegitable_data", error);
    res.status(500).json({ message: "Failed to insert data into vegitable_data", error });
  }
});
